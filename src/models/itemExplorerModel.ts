...
// ═══════════════════════════════════════════════════════
// Kept in step with the web's models/itemExplorer.ts — the same metrics, the
// same arithmetic and the same words, so an item reads the same on a phone.
//
...
import type {
  ...
  ItemPerformance,
} from './inventoryValuationModel';
import type { ReportDateRange } from './reportModel';
import { formatCurrency } from '../utils/formatters';

type ReportRange = ReportDateRange;

/** Money and quantities to the paisa, without float drift from a sum. */
const round2 = (n: number): number => Math.round(n * 100) / 100;
const sum = (values: number[]): number => round2(values.reduce((t, v) => t + v, 0));

const QTY = new Intl.NumberFormat('en-US', { maximumFractionDigits: 4 });
const formatQty = (n: number): string => QTY.format(n);

/** Rs 1,234.57 — the app's currency format everywhere else. */
const formatMoney = (n: number): string => formatCurrency(n, 'Rs ');
/** 1,234.57 — for a column whose header already says the currency. */
const formatAmount = (n: number): string => formatCurrency(n, '');

/** Rs 1.2M, Rs 340K, Rs 1.5K — with a real minus sign, like the web. */
...
/** Current against prior; no percentage from a zero base. */
const variance = (current: number, prior: number): { delta: number; percent: number | null } => {
...
export const periodValue = (
  ...
  if (key === 'marginPct') {
    const revenue = sum(known(column(months, 'revenue')));
    const gp = sum(known(column(months, 'grossProfit')));
    return revenue > 0 ? round2((gp / revenue) * 100) : null;
  }
  if (key === 'avgPrice') {
  ...
  return metric.unit === 'money'
    ? sum(values)
    : Math.round(values.reduce((t, v) => t + v, 0) * 10_000) / 10_000;
};
...
export const metricChange = (
  ...
  if (explorerMetric(key).unit === 'percent') {
    return { delta: Math.round((current - prior) * 10) / 10, percent: null };
  }
