...
// QuickBooks-inspired financial overview
// ═══════════════════════════════════════════════════════
...
import { MOCK_REVENUE } from '../../../network/mockAdminData';
...
const RevenueBarChart: React.FC<{
  ...
  const bars = data.map((d, i) => {
    ...
    return { ...d, heightPct, isLatest: i === data.length - 1 };
  });

  return (
    <View style={S.chartContainer}>
      {bars.map((bar) => (
        <View key={bar.month} style={S.barWrapper}>
          <Text style={S.barValue}>${bar.revenue}</Text>
    ...
    <View style={S.planRow}>
      ...
      <View style={S.planMeta}>
        <View style={S.planTopRow}>
          ...
          <Text style={S.planRevenue}>${revenue}/mo</Text>
        </View>
        ...
        <Text style={S.planSub}>{companies} companies · {pct}% of MRR</Text>
      </View>
    </View>
  );
};
...
const CompanyRow: React.FC<{
  ...
  <View style={S.coRow}>
    ...
    <View style={S.coRight}>
      <Text style={S.coRevenue}>${revenue}/mo</Text>
      {status !== 'active' && (
...
const RevenueAnalyticsScreen: React.FC = () => {
  ...
  const totalPlanRevenue = rev.byPlan.reduce((s, p) => s + p.revenue, 0);
      ...
      >
        ...
        >
          ...
          <View style={S.mrrLeft}>
            ...
            <Text style={S.mrrValue}>${rev.mrr.toLocaleString()}</Text>
            <View style={S.mrrGrowthRow}>
              <Feather name="trending-up" size={14} color="rgba(255,255,255,0.9)" />
              <Text style={S.mrrGrowthText}>+{rev.growthMoM}% from last month</Text>
            </View>
          </View>
          <View style={S.mrrRight}>
            <View style={S.mrrArrBox}>
              <Text style={S.mrrArrLabel}>ARR</Text>
              <Text style={S.mrrArrValue}>${(rev.arr / 1000).toFixed(1)}K</Text>
            </View>
          </View>
        </LinearGradient>
        ...
        <View style={S.metricsGrid}>
          ...
          <MetricCard
            ...
            value={`$${rev.avgRevenuePerAccount}`}
            icon="dollar-sign"
            color="#F59E0B"
            delay={160}
          />
        ...
        <View style={S.card}>
          <View style={S.cardHeader}>
            ...
            <View style={S.trendBadge}>
              ...
              <Text style={[S.trendBadgeText, { color: C.green }]}>+65%</Text>
            </View>
          </View>
          <RevenueBarChart data={rev.monthlyTrend} />
        </View>
        ...
        <View style={S.card}>
          <View style={S.cardHeader}>
            <View>
              ...
              <Text style={S.cardSub}>Breakdown of ${totalPlanRevenue} MRR</Text>
            </View>
          </View>
        ...
        >
          ...
          <View style={S.forecastContent}>
            ...
            <Text style={S.forecastSub}>
              At current growth rate, projected MRR next month is{' '}
              <Text style={{ fontWeight: '800', color: C.primary }}>
                ${Math.round(rev.mrr * (1 + rev.growthMoM / 100)).toLocaleString()}
              </Text>
            </Text>
          </View>
        </LinearGradient>
